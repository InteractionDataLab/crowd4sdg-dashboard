...
visPlot = function(subgraph, communities = rep(1, length(V(subgraph))), color = "auto", nodesize = 10, edgewidth = 0, title="", 
                   textsize = nodesize, edgelabel = rep(1, length(E(subgraph))), layout = "layout_nicely", directed = TRUE, hidden = rep(FALSE, ecount(subgraph)), seed = 123)
{
  ...
  colnames(edges)<-c("from","to","width", "label", "font.size", "color")
  plt = visNetwork(nodes, edges, height = "1000px", width = "100%",main = title)%>%
    visIgraphLayout(layout = layout, randomSeed = seed) %>% 
